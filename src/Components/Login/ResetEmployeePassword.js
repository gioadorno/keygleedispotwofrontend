...
import { changeEmployeePassword } from "../../actions/login";
...
const ResetPassword = () => {
    ...
    const submit = async () => {
        dispatch(changeEmployeePassword(token, { password: password.password, token: token })).then(() => setPasswordConfirm(true))
    };
