...
const Filters = ({ filterMonth, setFilterMonth, months, employees, filterEmployees, setFilterEmployees, year }) => {
    const changeMonth = (e) => {
  ...
  return (
    <Container sx={{ display: 'flex', alignItems: 'center', p: 2, bgcolor: 'white', boxShadow: 5, borderRadius: '1em'  }} maxWidth='md'>
        ...
        <FormControl sx={{ width: '50%' }}>
            <InputLabel id='selectMonth'>Select Month</InputLabel>
            <Select onChange={changeMonth} label='Select Month' value={filterMonth} id="selectMonth" >
                {months.map(month =>
                <MenuItem key={month + 1} value={month}>
                    {month.replace(year, '')}
                </MenuItem>
                    )}
            </Select>
